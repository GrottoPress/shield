...
module Avram
  ...
  module NestedSaveOperation
    ...
    macro has_one(type_declaration)
      ...
      {% nested_model_type =
        nested_type.superclass.name.gsub(/\:\:SaveOperation$/, "") %}

      {% assoc = T.resolve.constant(:ASSOCIATIONS).find do |assoc|
        assoc[:relationship_type] == :has_one &&
          assoc[:type].resolve.name == nested_model_type
      end %}
      ...
      def {{ nested_name }}
        return update_{{ nested_name }} unless id.value.nil?

        nested = create_{{ nested_name }}
      ...
      private def create_{{ nested_name }}
        {{ nested_type }}.new(
          {% for nested_attribute in nested_attributes %}
      ...
      private def update_{{ nested_name }}
        {{ nested_type }}.new(
