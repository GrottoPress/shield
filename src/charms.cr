...
module Avram
  ...
  module NestedSaveOperation
    ...
    macro has_one_create(type_declaration, *, assoc_name)
      {% nested_name = type_declaration.var %}
      {% nested_type = type_declaration.type %}

      {% nested_attributes = nested_type.resolve.constant(:ATTRIBUTES) %}
      {% nested_columns =
        nested_type.resolve.constant(:COLUMN_ATTRIBUTES).reject do |c|
