...
describe Shield::EndUserLoginsOnPasswordChange do
  it "logs out everywhere when password changes" do
    ...
    SaveUser.update(user, fake_params(user: {
  ...
  it "does not log other users out when password changes" do
    ...
    SaveUser.update(mary, fake_params(user: {
      password_digest: mary_new_password
    })) do |operation, _|
      operation.saved?.should be_true
