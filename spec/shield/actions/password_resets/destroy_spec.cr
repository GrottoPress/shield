...
describe Shield::PasswordResets::Destroy do
  it "ends password reset" do
    ...
    response = client.exec(
      PasswordResets::Destroy.with(password_reset_id: password_reset.id)
    )
  ...
  it "requires logged in" do
    response = ApiClient.exec(
      PasswordResets::Destroy.with(password_reset_id: 9)
    )
